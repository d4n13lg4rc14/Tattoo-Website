...
$calendar.= '<thead><tr class="bg-info"><th class="text-center" colspan="7"><a href="'. $_SERVER["PHP_SELF"].'?month='. $prev_month . '&year='.$prev_year.'" class="text-warning"><i class="fas fa-caret-left fa-lg"></i>&nbsp;</a>'.$monthName. " ". $year.'
<a href="'. $_SERVER["PHP_SELF"].'?month='. $next_month . '&year='.$next_year.'" class="text-warning"><i class="fas fa-caret-right fa-lg"></i>&nbsp;</a></td></tr>';
$headings = array('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday');
$calendar.= '<thead><tr class="bg-warning"><th>'.implode('</th><th>',$headings).'</th></tr></thead>';
...
for($list_day = 1; $list_day <= $days_in_month; $list_day++):
    $calendar.= '<td class="warning">';
        ...
        $calendar.= '<ul class="list-group"><li><a href="appointment.php?date='.$date.'&time='.$row["AppointmentTime"].'">'. $time . '</a></li></ul>';
        
        } else if ($date_time >= $today_time && $db_appointments != "" && $db_client_appointments == "") {
        $calendar.= '<ul class="list-group"><li><del style="color: red;">'. $time . '<del></li></ul>';

        } else if ($date_time >= $today_time && $db_appointments != "" && $db_client_appointments != ""){
        $calendar.= '<ul class="list-group"><li><strong style="color:blue">'. $time . '</strong><i class="fas fa-info-circle text-warning" data-toggle="tooltip" title="Your appointment with ' . $db_artist_appointment["FirstName"] . " " .  $db_artist_appointment["LastName"] . ' "></i>  '.  " "  . '<a href="delete.php?id='.$db_appointments["ID"] .'"><i class="fas fa-times-circle text-warning" data-toggle="tooltip" title="Delete"></i></a></li></ul>';

        } else if (($date_time < $today_time && $db_appointments == "" && $db_client_appointments == "") ||
                    ($date_time < $today_time && $db_appointments != "" && $db_client_appointments == "")) {
        $calendar.= '<ul class="list-group"><li><del>'. $time . '</del></li></ul>';  

        } else if ($date_time < $today_time && $db_appointments != "" && $db_client_appointments != "") {
            $calendar.= '<ul class="list-group"><li><del>'. $time . '</del> Your appointment with '. $db_artist_appointment["FirstName"]. " " . $db_artist_appointment["LastName"]. '</li></ul>'; 
        }
    }
    $calendar.= '</td>';
    if($running_day == 6):
